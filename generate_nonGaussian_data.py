...
tab_single = Table([beta_a1, beta_b1, beta_a2, beta_b2, omega_w1, omega_w2, df], names=['a1', 'b1', 'a2', 'b2', 'w1', 'w2', 'df'], masked=True)
...
tab_double = Table([beta_a1, beta_b1, beta_a2, beta_b2, omega_w1, omega_w2, df], names=['a1', 'b1', 'a2', 'b2', 'w1', 'w2', 'df'], masked=True)

tab = Table(vstack([tab_single, tab_double]), masked=True)
...
for ind, row in enumerate(tab):
    ...
    y = Xw @ beta + random.standard_t(row['df'], N)

    tab['t'][ind] = [t]
    tab['y'][ind] = [y]

#Add real world data
real_data = Table.read('/Users/clairesibbald/MScResearch/TIC_111010123_lightcurve.csv')
real_data['magnitude'] = -2.5*np.log10(real_data['flux']) + 20.44
real_data['magnitude_err'] = np.sqrt((-2.5/np.log(10)*(real_data['flux_err']/real_data['flux']))**2 + 0.05**2)
t = real_data['time']-np.min(real_data['time'])+0.01 # Shift to avoid error at t=0
y = real_data['magnitude']-np.mean(real_data['magnitude']) # Remove trend

real = [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, [t], [y]]
tab.add_row(real)

with h5py.File("~/msc-thesis/simulation_datasets/Simulation_Datasets_nonGaussian.h5", "w") as f:
